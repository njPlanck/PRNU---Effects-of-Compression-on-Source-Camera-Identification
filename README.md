...
### Conclusion
So far, our results indicate that while correlation values may have increased as images were aggressively compressed,they do not affect the matching accuracy. The matching accuracy remained stable for sigma filter for both high and low correlation vaules. Whilethe accuracy for gaussian filter declined at high compression levels, even with improved correlation values. Over all, this raises the question on effectiveness of a compression-only attack on SCI based on PRNU patterns. In particular, the results suggest that effectiveness of a compression-only attack is dependent more on the choice of extraction filter than on compression strength or coding scheme
