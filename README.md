...
* From the results, the matching accuracy for source identification with the sigma filter remains stable across different compression ratios and schemes. In fact, at higher compression ratios, the correlation values used for matching the right camera increase. This is consistent with theory, as compression tends to remove scene information leaving compression artifacts. So the as scene is progressively replaced by compression artifacts, the PRNU start to capture these artifacts approximating more to the reference pattern.
* It is also important to note that compression algorithms like JPEG exploit the redundancy for data reduction. From the sample compressed with JPEG attached above, blocky artifacts start to appear in low-frequency, uniform regions of the image as it was aggressively compressed to 100KB, while still preserving the high-frequency, edge structure. So the suppression of high frequencies during compression may not be the only reason why SCI systems fail under compression attacks. This is because sigma filter by its very nature preserve non linear high frequency noise as edges. And a purely low-pass effect from compression would reduce its robustness rather than improve it, as observed in the results. 

Limitations of Gaussian Filter:
* The gaussian filter also achieved good source identification under mild compression (3MB). But its accuracy sharply declines under stronger compression. This declined followed the reduction in the quality of the compressed images. In some cases (e.g., JP2 or JXL at 100KB), accuracy to 35%. This is even when the correlation values improved mildly with aggressive compression, suggesting that while what is left after compression is closer to the reference pattern, this does not necessarily improve the matching accuracy.

* The drop in accuracy can be attributed to the strength of smoothing from gaussian filtering. The filter allows a lot of scene information to contaminate the extracted PRNU pattern. And as the scene is removed during compression and replaced with compression artifacts, the filter start to capture the more of these artifacts from the scene. Unlike sigma filter, the gaussian filter captures more of these compression, weakening the PRNU pattern extracted. This explains why the accuracy not improving with mild increase in correlation. 
...
### Conclusion
So far, our results indicate that while correlation values may have increased as images were aggressively compressed, they do not affect the matching accuracy for sigma filter. While the accuracy for gaussian filter declined at high compression levels, even with improved correlation values. Of ocurse we recognise that this experiment was done on a very small sample data. It would be interesting to see how this scales for larger dataset. But over all, this raises a question on the effectiveness of a compression-only attack on SCI based on PRNU patterns. In particular, these results suggest that effectiveness of a compression-only attack is dependent more on the choice of extraction filter than on compression strength or coding scheme.
