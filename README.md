...
* It is also known that compression algorithms like JPEG exploit the redundancy in data for its reduction. From the sample compressed with JPEG attached above, blocky artifacts start to appear in low-frequency, uniform regions of the images as they were aggressively compressed to 100KB, while still preserving the high-frequency, edge structure. So the suppression of high frequencies during compression may not be the only reason why SCI systems fail under compression attacks. Since the sigma filter rely on these high frequency noise to extract PRNU patterns. A purely low-pass effect from compression logically reduces its robustness, contrary to the observed results.
...
* The drop in accuracy can be attributed to the strength of smoothing from the filtering process. The filter captures more scene information alongside the extracted PRNU pattern. And as the scene is removed during compression and replaced with compression artifacts, the filter capture more of these compression artifacts which is not camera specific. These reduces the robustness of the reference pattern that is built from compressed images, which leads to the poor results when used for SCI.
